    ...
    \begin{table}[]
        \resizebox{0.75\textwidth}{!}{%
            \begin{tabular}{l|l|l|l}
                ...
                \hline
                \pause Bubble Sort    & $O(n)$          & $O(n^2)$       & $O(1)$         \\
                \pause Insertion Sort & $O(n^2)$        & $O(n^2)$       & $O(1)$         \\
                \pause Selection Sort & $O(n)$          & $O(n^2)$       & $O(1)$         \\
                \pause Merge Sort     & $O(n\log{n})$   & $O(n\log{n})$  & $O(n\log{n})$  \\
                \pause Heap Sort      & $O(n\log{n})$   & $O(n\log{n})$  & $O(n\log{n})$  \\
                \pause Quick Sort     & $O(n\log{n})$   & $O(n\log{n})$  & $O(n^2)$       \\
                \pause Tree Sort      & $O(n\log{n})$   & $O(n\log{n})$  & $O(n^2)$       \\
                \pause Counting Sort  & $O(n+k)$        & $O(n+k)$       & $O(n+k)$       \\
                \pause Bucket Sort    & $O(n+k)$        & $O(n+k)$       & $O(n^2)$       \\
                \pause Radix Sort     & $O(d\times n)$  & $O(d\times n)$ & $O(d\times n)$
            \end{tabular}%
        }
    \end{table}
\end{frame}
